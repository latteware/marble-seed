...
const utils = new nunjucks.Environment()

utils.addFilter('toWidget', function (type, count) {
  if (type === 'number') { return 'NumberWidget' }
  if (type === 'date') { return 'DateTimeWidget' }
    ...
    return new Promise(function (resolve, reject) {
      fs.readFile(templatePath, 'utf8', (err, data) => {
        if (err) {
          ...
          const template = nunjucks.compile(data, utils)
