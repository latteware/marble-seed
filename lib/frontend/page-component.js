...
class ErrorBoundary extends Component {
  ...
  render () {
    ...
    if (hasError) {
      return <ErrorPage type='render' message={error.message} />
    } else {
...
class PageComponent extends Component {
  static config (config) {
    this._config = _.merge(_.clone(this._config || {}), config)
  }
  ...
  static asRouterItem (props) {
    ...
    if (this.prototype.getFilters) {
      const getFilters = this.prototype.getFilters.bind({ state: {} })
      const filters = getFilters()

      if (filters && filters.uiSchema) {
        config.filterType = 'react-schema-form'
        console.warn(`[Deprecation notice for v0.7.0]On ${this.name} getFilters use react-schema-form, migrate to marble-form config.`)
      }
    }
  ...
  constructor (props) {
    ...
    this.config = this.constructor._config || {}

    this.state = this.baseState

    if (!this.config.name) {
      this.config.name = uuidv4()
    }
  ...
  getBasicStates () {
    ...
    if (loadError) {
      return <ErrorPage message={loadError.message} {...loadError} />
    }
  }
}
